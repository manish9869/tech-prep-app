    ...
    const handleFileChange = async (e) => {
        ...
        for (const row of parsed) {
            ...
            const payload = {
                ...
                correct_option_index: Number.isInteger(row.correct_option_index) ? row.correct_option_index : 0,
                reference_links: row.reference_links || '',
                round: ['screening', 'technical', 'manager', 'hr'].includes(row.round) ? row.round : null,
                tags: Array.isArray(row.tags) ? row.tags : (row.tags ? row.tags.split(';').map(t => t.trim()).filter(Boolean) : []),
                company_tags: Array.isArray(row.company_tags) ? row.company_tags : (row.company_tags ? row.company_tags.split(';').map(t => t.trim()).filter(Boolean) : []),
